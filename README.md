...
**Statut : prototype jouable — modèle v0.6 « corruption cachée ».** Tu te finances **en secret** (les autres voient ton argent monter, pas s'il est propre ou sale) ; la **dénonciation est un pari** qui frappe l'argent ; les voix viennent des votants + des **coalitions** ; chaque joueur a un **rôle secret** (pouvoir + objectif). Calibré par simulation (~11 manches, **≥ 98 %** des parties par le seuil, 2→6 joueurs). Spec complète : `docs/REGLES.md §16`.
...
`catalogue.html` reste disponible pour le **🃏 Catalogue** de toutes les cartes (généré depuis le moteur) et le **📊 bac à sable d'équilibrage** (Monte Carlo).
...
- **Seuil de victoire = 45 voix (FIXE, tous les N)** — calibré par le `simulate()` réel v0.7 : **100 % des parties par le seuil**, **~14-15 manches** quel que soit N (un seuil fixe en voix garde un nombre de manches ~constant → chaque joueur joue à peu près autant de tours). Réglable dans `src/engine.js` (`const SEUIL_VOIX = 45` ; 40 pour un cran plus rapide).
- Coûts d'attaque : **dénoncer 2 M€** (+ **amende 3 M€** si raté) · **Débauchage / OPA** = cartes (la victime cède un bloc).
